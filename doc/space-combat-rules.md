...
> **Multi-battery sensor sharing** (B3 p.56, issue #40): *"For ship's with multiple batteries, a sensor operator can provide the firing solution for a number of targets equal to their Electronics (sensors) skill level. If there is more than one sensor operator, they can direct multiple batteries."* Verificato: **già modellato correttamente** dal budget azioni generico per-ruolo (§1/issue #19) — `sensor_operator` è l'unico ruolo Sensor-side **non** cappato a 1 (`buildActionBudget`: `Math.max(0, skill)`, a differenza di Gunnery), quindi un Sensor Operator con skill N ha già N azioni/round, e ogni Step 1 rieseguito da zero ne consuma una — esattamente "provide firing solution for N targets = skill level", senza bisogno di un meccanismo di "banking" del risultato tra più tiri Gunner. Nessuna modifica necessaria per il caso a un solo Sensor Operator. **Limite noto, non risolto qui**: la seconda clausola ("if more than one sensor operator") non è rappresentabile — `crewAssignments` ha un solo slot `sensor_operator` per nave, quindi due Sensor Operator simultanei sulla stessa nave non esistono nel modello dati. In pratica lo scenario multi-batteria non è comunque raggiungibile oggi per un motivo diverso e più a monte: `gunner_bay` (il secondo ruolo Gunner pensato apposta per una seconda batteria) non è mai cablato in `AttackModal.jsx`, che ha `gunner_turret` hardcoded ovunque indipendentemente dal weapon slot scelto — una nave non può quindi mai sparare da due batterie nello stesso round. Tracciato come issue separata (fuori scope per #40).
...
- **Electronic Warfare** — Difficult (10+) Electronics (comms): disturba i lock del bersaglio. Successo: Effect 1–4 → il bersaglio subisce **DM−1** a Gunner questo round; Effect 5–6 → **DM−2**; Effect ≤−5 → il bersaglio ottiene invece **DM+1** (ha triangolato le emissioni del jammer). Il DM è salvato come `ewEffect` sulla nave bersaglio (`ewTarget` punta alla nave che sta subendo il jam) e applicato automaticamente allo step 3 Gunner. // B3 p.54
  - **Encrypted Comms** (B3 p.58, issue #66): *"Encrypted comms provide DM+2 to the target of the check"* — è un check **opposto** nel testo, ma questo motore semplifica EW a un singolo tiro contro soglia fissa (stesso pattern di `evasionDm`), quindi il DM+2 del bersaglio è ripiegato come **DM−2 al tiro dell'attaccante** in `ActionModal.jsx`. Toggle `ship.encryptedComms` in `ShipDetailModal.jsx` (sezione COMMUNICATIONS), non è un modificatore di Signature.
- **Scan Target** — Routine (8+) Electronics (sensors), DM−1 per fascia di distanza. Effect 1–3: info di base alla fascia attuale (tabelle Sensor Target/Sensor Detail, Trav CRB p.160 — B3 p.54 cita erroneamente p.151, stessa famiglia di errore del trait Auto, issue #60). Effect 4–5: info come se il bersaglio fosse una fascia più vicino. Effect 6: come se fosse due fasce più vicino. **Puramente informativo** — nessuna mutazione di stato, il GM narra il risultato leggendo l'Effect dal roll banner. // B3 p.54
