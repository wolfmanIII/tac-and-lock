...
export function DroneLaunchModal({ payload, onClose }) {
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <p className="text-[10px] font-mono text-gunmetal-500">2300AD B3 p.61 — each unit tracked individually</p>

      <div className="grid grid-cols-2 gap-2">
        <div>
          <p className="text-[10px] font-display text-gunmetal-500 tracking-widest mb-1">LAUNCHER</p>
          <p className="text-sm font-mono text-gunmetal-200">{attacker.profile?.name}</p>
        </div>
        <div>
      ...
      {droneWeapons.length === 0 ? (
        ...
        <div className="space-y-1">
          ...
          <select value={weaponIdx} onChange={(e) => setWeaponIdx(Number(e.target.value))}
            className="w-full bg-gunmetal-800 border border-gunmetal-600 text-gunmetal-200 font-mono text-sm rounded px-2 py-1.5 focus:border-amber-400 outline-none">
      ...
      {weapon && (
        <div className="bg-gunmetal-800/50 rounded p-3 grid grid-cols-3 gap-2">
          <div>
            <p className="text-[10px] font-display text-gunmetal-500 tracking-widest">LAUNCH BAND</p>
            <p className="text-sm font-mono text-gunmetal-200">{band}</p>
          </div>
          <div>
            <p className="text-[10px] font-display text-gunmetal-500 tracking-widest">TAC SPEED</p>
            <p className="text-sm font-mono text-gunmetal-200">{weapon.tacSpeed ?? '—'}</p>
          </div>
          <div>
            <p className="text-[10px] font-display text-gunmetal-500 tracking-widest">ENDURANCE</p>
            <p className="text-sm font-mono text-gunmetal-200">{weapon.enduranceRounds ?? '—'}R</p>
          </div>
        </div>
      )}

      <div>
        <p className="text-[10px] font-display text-gunmetal-500 tracking-widest mb-2">UNITS TO LAUNCH</p>
        <div className="flex gap-2">
          {[1,2,3,4,5,6].map((n) => (
            <button key={n}
              className={`flex-1 py-2 text-sm font-mono border rounded ${count === n ? 'border-amber-500 text-amber-300 bg-amber-900/30' : 'border-gunmetal-700 text-gunmetal-400 hover:border-gunmetal-500'}`}
              onClick={() => setCount(n)}>{n}</button>
          ))}
        </div>
        <p className="text-[9px] font-mono text-gunmetal-600 mt-1">Each unit closes range and attacks independently — no salvo bonus.</p>
      </div>
