...
| Auto X | Selettore Single/Burst/Full Auto in `AttackModal.jsx` — CRB p.75. Burst: +X danno (un tiro). Full Auto: X tiri di danno separati contro lo stesso bersaglio (armatura applicata per tiro) | B3 (ref CRB) |
...
├── utils/
│   ├── combat.js                 ← getWeaponTraitAttackDm, getPointDefenceDm, computeEffectiveSignature, computeAttackDMs, rollDamage, getAutoScore, rollFullAuto, crits
